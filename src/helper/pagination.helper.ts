...
export const createPaginationOptions = (req) => {
  ...
  export const createPaginationOptionsFromBody = (req) => {
    let page = parseInt(req.page, 10) || 1;
    let limit = parseInt(req.limit, 10) || 10;
    let skip = parseInt(req.skip, 10) || 0;
    const pagination = new PaginationOptions();
  ...
  export const orderNumbering = (pagination) => {
    let num = 0;
    if (pagination.page > 1) {
      num = pagination.limit * (pagination.page - 1);
    }
